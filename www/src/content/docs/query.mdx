...
Simple stack query provides global utilities to make your Astro components interactive. To get started, apply the `<RootElement>` wrapper around your Astro component markup:
...
Next, apply the `data-target` attribute to the element you want to target. Use the global `$()` function to apply the target value. This will generate a scoped identifier to prevent conflicts with other components in your project:
...
Now create a `<script>` tag, and use the `RootElement.ready()` wrapper to define your client script. `.ready()` provides the `$()` function to select your `btn` element. This returns a web-standard [`HTMLElement`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement) you can use to, say, add an event listener with `.addEventListener()`:
...
`RootElement.ready()` provides the `$()` function to select elements. This will find the first match based on `data-target`, and throw if no match is found.

The result will be a standard [`HTMLElement`](https://developer.mozilla.org/en-US/docs/Web/API/HTMLElement) by default. To narrow this type to a particular element, you can pass a type generic to the `$()` function:
...
The simplest place to store state values is on an element itself. For example, you may track the `open` state on a dropdown by toggling a data attribute. Call `.toggleAttribute()` when a button is clicked, and adjust the dropdown's `visibility` style from `invisible` to `visible` based on the value:
...
<RootElement>
  <button data-target={$('toggle')}>Toggle</button>
  <ul class="data-[open]:visible invisible" data-target={$('drawer')}>
    <li>Item 1</li>
  </ul>
</RootElement>
...
```astro "const count = new Signal.State(0)"
<script>
...
You can update this value using `.set()`, and retrieve the current value using `.get()`. This example increments a counter when the `btn` element is pressed:
