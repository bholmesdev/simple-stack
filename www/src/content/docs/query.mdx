...
Simple Query is an Astro integration. You can install using the `astro add` CLI:
...
You can retrieve this value from the client using the `data` object. This is available from the `ctx` object passed as the second argument by `RootElement.ready()`.
